...
def _retrieve_for_messages(
    ...
    return get_rag_pipeline().retrieve_context(query, llm_config=llm_config, top_k=5)
...
async def langchain_chat_stream(
    ...
    langchain_messages = _to_langchain_messages(messages, system_prompt, retrieve_result, session_context)
...
async def chat_stream_with_tools(
    ...
    # 联网搜索:生成前强制走一遍 Tavily，把网页结果编号注入 prompt。
    web_search_block = ""
    if web_search:
    ...
    try:
        ...
        final_text = clean_model_answer(_message_content_to_text(getattr(response, "content", "")))
        if final_text:
            async for event in _yield_text_delta(final_text):
                yield event
        else:
            fallback_model = create_chat_model(llm_config)
            fallback_messages = [
                *langchain_messages,
                HumanMessage(content="请基于上面的工具返回结果，用中文给用户一个简洁的最终回答，不要再调用任何工具。"),
            ]
            last_usage: Optional[dict[str, Any]] = None
            with llm_duration_timer(llm_config.model_name, "langchain_tools_final"):
                async for chunk in fallback_model.astream(fallback_messages):
                    usage = extract_usage_metadata(chunk)
                    if usage:
                        last_usage = usage
                    chunk_text = clean_model_answer(_message_content_to_text(chunk.content))
                    if chunk_text:
...
async def chat_stream(
    ...
    async for event in langchain_chat_stream(messages, llm_config, system_prompt, knowledge_source):
        yield event
