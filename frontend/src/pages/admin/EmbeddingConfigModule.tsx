...
import { Binary, Check, CheckCircle, KeyRound, Loader2, Pencil, Plus, Save, ToggleRight, Trash2, Wifi, X } from 'lucide-react'
import {
...
import type { EmbeddingConfig, TestResult } from '../../types'
...
const providerPresets = [
  { provider: 'hash', model_name: 'hash-384', label: 'Hash fallback' },
  { provider: 'openai', model_name: 'text-embedding-3-small', label: 'OpenAI small' },
  { provider: 'openai', model_name: 'text-embedding-3-large', label: 'OpenAI large' },
  { provider: 'gemini', model_name: 'models/gemini-embedding-001', label: 'Gemini' },
]

export default function EmbeddingConfigModule() {
  ...
  async function loadConfigs() {
    setLoading(true)
    try {
      setConfigs(await listEmbeddingConfigs())
      setError('')
    } catch (err) {
      setError(err instanceof Error ? err.message : '向量模型配置加载失败')
    } finally {
      setLoading(false)
    }
  }
  ...
  function openEdit(config: EmbeddingConfig) {
    setForm({
  ...
  function applyPreset(preset: (typeof providerPresets)[number]) {
    setForm((current) => ({
      ...
      api_key: preset.provider === 'hash' ? '' : current.api_key,
      base_url: preset.provider === 'hash' ? '' : current.base_url,
    }))
  }
  ...
  async function handleSave(event: FormEvent) {
    ...
    try {
      ...
      if (editingId === null) {
        await createEmbeddingConfig({ ...form, is_active: shouldActivate || configs.length === 0 })
      } else {
        const updates: Partial<typeof form> & { is_active?: boolean } = { ...form }
        if (shouldActivate) updates.is_active = true
        await updateEmbeddingConfig(editingId, updates)
      }
      setSaveStatus('saved')
      closeForm()
      await loadConfigs()
      ...
      setError(err instanceof Error ? err.message : '向量模型配置保存失败')
      setSaveStatus('error')
    }
  }
  ...
  async function handleTestConnection() {
    ...
    setTestResult(null)
    try {
      const result = await testEmbeddingConfig(form)
  ...
  return (
    <div className="space-y-6">
      <ModuleCard
        icon={<Binary size={20} />}
        title="向量模型配置"
        desc="只管理 RAG Embedding 模型、API Key 和 Base URL。切换启用项后会刷新后端 RAG 运行时，旧文档需要在知识库模块重建向量。"
        actions={
      ...
      >
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <Metric label="当前向量模型" value={active ? `${active.provider} / ${active.model_name}` : '未配置'} />
          <Metric label="配置数量" value={`${configs.length}`} />
          <Metric label="端点类型" value={active?.base_url ? '自定义端点' : '默认端点'} />
        </div>

        <InlineNotice>
          更换向量模型后，请到“知识库文件”模块点击“重建向量”，把已有 chunks 重新写入新的 Chroma collection。
        </InlineNotice>
        ...
        <Modal
          open={showForm}
          onClose={closeForm}
          title={editingId === null ? '新建向量模型配置' : '编辑向量模型配置'}
          subtitle={editingId === null ? '添加一个新的 Embedding 供应商配置。' : '修改已保存的向量模型参数。'}
        >
          <form onSubmit={handleSave}>
              ...
              {providerPresets.map((preset) => (
                <button
                  ...
                  className={`rounded-2xl px-3 py-1.5 text-xs transition ${
                    form.provider === preset.provider && form.model_name === preset.model_name
                      ? 'bg-ink text-paper'
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Provider">
                <input
                  ...
                  className="input"
                  placeholder="openai / gemini / hash"
                />
              </Field>
              ...
              <Field label="Base URL" hint="OpenAI-compatible 供应商可填写自定义地址；OpenAI/Gemini 官方可留空。">
                <input
                  value={form.base_url}
              ...
              <Field label="API Key">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={form.provider === 'hash' ? 'hash 不需要 API Key' : 'sk-...'}
                  />
                </div>
              </Field>
            </div>
            ...
            <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-3xl bg-[#F8F6F1] px-4 py-3 transition hover:bg-clay/20">
              ...
              />
              <span className="text-sm text-ink">保存后立即启用此向量模型</span>
        ...
        <div className="mt-5">
          {loading ? (
            ...
            <EmptyState>还没有向量模型配置。默认 hash fallback 会由后端自动创建。</EmptyState>
          ) : (
